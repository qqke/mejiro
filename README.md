...
3. 已经部署过 v1 的项目，依次执行 `supabase/v2_v3_migration.sql`、`supabase/v4_v5_migration.sql`、`supabase/v6_v7_migration.sql`。
4. 已经部署到 v3 的项目，依次执行 `supabase/v4_v5_migration.sql`、`supabase/v6_v7_migration.sql`。
5. 已经部署到 v5 的项目，只需要执行 `supabase/v6_v7_migration.sql`。
6. 参考 `.env.example` 创建本地 `.env`。
7. 在 Supabase Dashboard 的 Authentication / Users 中创建用户。
8. 按需在 `public.profiles` 中调整用户角色。
